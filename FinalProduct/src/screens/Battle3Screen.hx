...
class Battle3Screen extends Screen
{
	...
	{
		enemyData = Assets.getBitmapData( "img/Baby_Enemy.png");
		enemy = new Bitmap( enemyData );
		enemy.scaleX = enemy.scaleY = 1 / 4;
		enemy.x = 500;
		enemy.y = 240;
		addChild(enemy);
	}
	...
	{ 
		...
		for ( column in 0...5 )
		{
			...
			{
				Main.instance.loadScreen (ScreenTypes.Dead3);
			}
			...
			{
				Main.instance.loadScreen (ScreenTypes.Cutscene8);
			}
		} 
		addChild(box);
	}	
	...
	function enemyTurn()
	{
		...
		{
			enemyToPlayerDamage = Math.ceil( 10 + (15 * Math.random()) );
