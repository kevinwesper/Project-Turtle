...
class Battle1Screen extends Screen
{
	...
	{
		enemyData = Assets.getBitmapData( "img/HagridHamster.png");
...
/*			var attackType : Int = Math.ceil( 5 * Math.random() );
			if ( attackType == 1 || attackType == 2 || attackType == 3 || attackType == 4 || attackType = 5)
			{
				enemyToPlayerDamage = Math.ceil( 20 * Math.random() );
				Actuate.tween (enemy, 1, { x: (player.x + 125), y: player.y }).repeat (1).reflect ();
				selection.pop();
				isPlayerAlive(0, enemyToPlayerDamage);
				playerTurn();
			}
			else
			{
				enemyToEnemyDamage = Math.ceil( (-20) * Math.random() );					// testing
				Actuate.tween (enemy, 1, { x: (player.x + 125), y: player.y }).repeat (1).reflect ();
				selection.pop();
				isEnemyAlive(enemyToEnemyDamage, 0);
				playerTurn();
			}*/
		}
	}
